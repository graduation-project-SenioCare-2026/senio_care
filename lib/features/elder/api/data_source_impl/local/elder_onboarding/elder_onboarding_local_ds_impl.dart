...
class ElderOnboardingLocalDsImpl implements ElderOnboardingLocalDs {
  ...
  Future<Result<List<AllergyEntity>>> getAllergies() {
    return safeCall(() async {
      ...
      return allergiesList
          .map(
            (json) =>
                AllergyModel.fromJson(json as Map<String, dynamic>).toEntity(),
          )
          .toList();
    });
  }
  ...
  Future<Result<List<DiseaseEntity>>> getDiseases() {
    return safeCall(() async {
      ...
      return diseasesList
          .map(
            (json) =>
                DiseaseModel.fromJson(json as Map<String, dynamic>).toEntity(),
          )
          .toList();
    });
  }
  ...
  Future<Result<List<MobilityStatusEntity>>> getMobilityStatus() {
    return safeCall(() async {
      ...
      final List<dynamic> mobilityStates =
          jsonContent['mobility_status'] as List;
      return mobilityStates
          .map(
            (json) => MobilityStatusModel.fromJson(
              json as Map<String, dynamic>,
            ).toEntity(),
          )
          .toList();
